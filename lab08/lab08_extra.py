...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for input in typing_input:
            result.append(self.press(input))
        return ''.join(result)
    ...
    while sum_first != sum_second:
        if sum_first < sum_second:
            ...
            if n > len(second):
                break
            sum_second = sum(second[:n])
    do_trade = sum_first == sum_second

    if do_trade:  # change this line!
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def to_string(link):
        ...
        if link.first is Link:
            result += to_string(link.first)
        else:
            result += str(link.first)
        result += str(mid)
        result += to_string(link.rest)
        result += str(back)
        return result
    return to_string
