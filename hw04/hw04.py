...
def squares(s):
    ...
    def get_perfect_square_root(x):
        return (
            [v for v in range(1, round(x / 2 + 1)) if v * v == x][0]
            if len([v for v in range(1, round(x / 2 + 1)) if v * v == x]) > 0
            else None
        )

    return (lambda x: [v for v in x if v is not None])(
        [get_perfect_square_root(v) for v in s]
    )
...
def g_iter(n):
    ...
    if n <= 3:
        return n
    gn1, gn2, gn3, i = 3, 2, 1, 3
    g = lambda: gn1 + 2 * gn2 + 3 * gn3
    while i < n - 1:
        gn1, gn2, gn3 = g(), gn1, gn2
        i += 1
    return g()
