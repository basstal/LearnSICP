...
def move_stack(n, start, end):
    ...
    intermediation = 1 if start != 1 and end != 1 else 2 if start != 2 and end != 2 else 3
    if n > 1:
        move_stack(n - 1, start, intermediation)
    print_move(start, end)
    if n > 1:
        move_stack(n - 1, intermediation, end)
...
def balanced(m):
    ...
    if not is_mobile(m):
        return True
    mobile_sides = sides(m)
    return length(mobile_sides[0]) * total_weight(end(mobile_sides[0])) == length(mobile_sides[1]) * total_weight(end(mobile_sides[1])) and balanced(end(mobile_sides[0])) and balanced(end(mobile_sides[1]))
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        count, balancing = 0, self.balance
        while balancing < amount:
            balancing += balancing * self.interest
            count += 1
        return count
...
def check_par():
    ...
    r2 = interval(1, 20)  # Replace this line!
    return r1, r2
