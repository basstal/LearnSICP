...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(procedure, rest.map(lambda v: v if scheme_numberp(v) else scheme_eval(v, env)), env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    first_result = scheme_eval(expressions.first, env)
    return first_result if expressions.second == nil else eval_all(expressions.second, env)
    # END PROBLEM 8
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        second_val = expressions.second
        env.define(target, second_val.first if scheme_numberp(second_val.first) else scheme_eval(second_val.first, env))
        return target
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    eval_expr = expressions.first
    eval_result = scheme_eval(eval_expr, env)
    if scheme_falsep(eval_result):
        return eval_result
    return eval_result if expressions.second == nil else do_and_form(expressions.second, env)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    eval_expr = expressions.first
    eval_result = scheme_eval(eval_expr, env)
    if scheme_truep(eval_result):
        return eval_result
    return eval_result if expressions.second == nil else do_or_form(expressions.second, env)
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    check_formals(bindings.map(lambda v: v.first))
    return env.make_child_frame(bindings.map(lambda v: v.first), bindings.map(lambda v: scheme_eval(v.second.first, env)))
    # END PROBLEM 15
